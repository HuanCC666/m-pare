...
# This file lives under `pare/scenarios/generator/prompt/`.
# We want the top-level `pare/` package directory for filesystem hints in prompts.
PARE_DIR = Path(__file__).resolve().parents[3]
...
PROJECT_CONTEXT_SUMMARY = textwrap.dedent(
    f"""\
    - Proactive Agents Research Environment (PARE) extends Meta-ARE with stateful, navigation-aware app wrappers so LLM planners can reason about realistic mobile workflows.
    - Scenarios seed deterministic baseline data inside PARE apps, then drive environment + oracle events so the proactive agent can infer goals and act.
    - The multi-step generator edits a single scenario file cloned from the PARE seed template; each step agent only touches its dedicated TODO block and preserves WARNING comments.
    - Use provided helpers (`get_typed_app`, `EventRegisterer`, `ScenarioValidationResult`) and keep PARE plus Meta-ARE APIs aligned with the state they expose.
...
GLOBAL_CONTEXT_PROMPT = textwrap.dedent(
    ...
    You operate as the coordinated prompt stack that powers PARE's multi-step scenario authoring pipeline. Keep outputs concise, deterministic, and aligned with the available PARE tooling.
    ...
    - Non-oracle environment events MUST map to functions declared in `pare/apps/notification_templates.py` for both user and agent streams.
...
APP_INITIALIZATION_SNIPPETS = {
    ...
    "PAREAgentUserInterface": {
        "init": "self.agent_ui = PAREAgentUserInterface()",
        "flow": "aui = self.get_typed_app(PAREAgentUserInterface)",
...
APP_IMPORT_INSTRUCTIONS = {
    "HomeScreenSystemApp": {
        "import instruction": "from pare.apps import HomeScreenSystemApp",
    },
    "PAREAgentUserInterface": {
        "import instruction": "from pare.apps import PAREAgentUserInterface",
    },
    "StatefulEmailApp": {
    ...
    "StatefulContactsApp": {
        "import instruction": {
            "from pare.apps import StatefulContactsApp",
            "from are.simulation.apps.contacts import Contact",
        },
    },
    "StatefulMessagingApp": {
        "import instruction": "from pare.apps import StatefulMessagingApp",
    },
    "StatefulShoppingApp": {
        "import instruction": "from pare.apps.shopping import StatefulShoppingApp",
    },
    "StatefulCabApp": {
        "import instruction": "from pare.apps.cab import StatefulCabApp",
    },
    "StatefulApartmentApp": {
        "import instruction": "from pare.apps.apartment import StatefulApartmentApp",
    },
    "StatefulNotesApp": {
        "import instruction": "from pare.apps.note import StatefulNotesApp",
    },
    "StatefulReminderApp": {
        "import instruction": "from pare.apps.reminder import StatefulReminderApp",
    },
}
...
_SCENARIO_DESCRIPTION_BODY = textwrap.dedent(
    """\
    You are the Step 1 description agent for the PARE multi-step generator.
    Produce:
    ...
    - The trigger(s) you describe must be representable using methods that have notification templates in `pare/apps/notification_templates.py`.
...
_SCENARIO_UNIQUENESS_BODY = textwrap.dedent(
    f"""\
    You review new scenario descriptions to ensure they are unique compared to existing PARE scenarios.
    Consider triggers, cross-app interactions, constraints, and tool usage patterns.

    Before deciding, you may use the Read tool to:
    - List and inspect existing PARE user scenarios under: {SCENARIOS_DIR_DISPLAY}
    ...
    - The scenario metadata file you are asked to read may be filtered to include only scenarios that use the same core app combination
      as the current run (excluding PAREAgentUserInterface and HomeScreenSystemApp which are always present).
...
_APPS_AND_DATA_BODY = textwrap.dedent(
    ...
    Using the approved narrative, list the baseline data to seed each PARE app inside `init_and_populate_apps()`.
    ...
    - Use the PARE app classes under `pare/apps/` and their Meta-ARE bases under `are/simulation/apps/` as the source of truth for which methods exist; do not invent new APIs.
    ...
    - When you need to check available methods or attributes on an app, use the Read tool to open the corresponding Python files in the PARE apps directory
    ...
    Inheritance-aware API lookup (applies to ALL PARE apps):
    - PARE apps are thin stateful wrappers around Meta-ARE base apps. For example:
      - `StatefulEmailApp` is defined in `pare/apps/email/app.py` and inherits from `EmailClientV2` in `are/simulation/apps/email_client.py`.
    - When you need to know what methods/fields exist on a PARE app, you MUST:
      1. Use Read to open the PARE wrapper file under `pare/apps/.../app.py` (e.g., `StatefulEmailApp`).
    ...
    - Do NOT assume that a method exists on a PARE app because you have seen similar names elsewhere. Only call methods that you have verified
      either on the PARE wrapper class itself or on one of its explicit base classes in the Meta-ARE codebase.
    ...
    - Before seeding messaging history, Read the PARE messaging wrapper and Meta-ARE base:
      - `pare/apps/messaging/app.py` (Stateful wrapper)
    ...
    - Read the defining source file for that app/object under `pare/apps/` and/or `are/simulation/apps/`.
...
_EVENTS_FLOW_BODY = textwrap.dedent(
    ...
    - REQUIRED (template coverage): every non-oracle environment event MUST call a method that has a notification template entry
      in `pare/apps/notification_templates.py` for both user and agent streams (the NOTIFICATION_TEMPLATES dict).
    ...
    - REQUIRED (write actions must be user-gated; CRITICAL):
      ...
      - Implementation rule:
        1) Create a proposal via `PAREAgentUserInterface.send_message_to_user(...)`.
        2) Create a user acceptance via `PAREAgentUserInterface.accept_proposal(...)`.
    ...
    - REQUIRED (explicit environment cue for every proposal):
      - Every agent proposal sent via `PAREAgentUserInterface.send_message_to_user(...)` MUST be justified by at least one concrete NON-ORACLE environment cue
    ...
    - REQUIRED (no private methods in event flow):
      - Do NOT call any private/underscore-prefixed app method (e.g., `_get_or_create_default_conversation`, `_create_conversation`) inside `build_events_flow()`.
        Use only verified public tool APIs from PARE wrappers / Meta-ARE bases.
    ...
    - For non-oracle environment events, only use methods that have notification templates in `pare/apps/notification_templates.py` (see the NOTIFICATION_TEMPLATES dict),
      and make sure at least one such env event appears BEFORE any oracle events.
    - For oracle/user actions, use app tools defined on the PARE apps and their Meta-ARE bases; do not invent new methods.
    ...
    - Before calling ANY app method or constructing ANY app-defined object (PARE or Meta-ARE), you MUST use the Read tool to open the
      defining Python source file for that class/method and copy the exact signature + required arguments.
      - PARE app wrappers live under `pare/apps/` (e.g., `pare/apps/<app_name>/app.py` or similar).
      - Meta-ARE base apps live under `are/simulation/apps/`.
    - Inheritance rule for PARE apps:
      - PARE apps like `StatefulEmailApp`, `StatefulMessagingApp`, etc. are usually declared as subclasses of Meta-ARE bases.
      - You MUST inspect BOTH:
        1. The PARE wrapper file under `pare/apps/.../app.py` to see the wrapper class definition.
      ...
      - Only treat methods as available on the PARE app if they are defined either directly on the wrapper class or on one of its explicit
        base classes you have opened with Read.
    - Navigation state tools:
      - Some PARE apps (like email) also expose `user_tool()` methods on navigation state classes under `pare/apps/<app_name>/states.py`
    ...
    Go back to source (`pare/apps/` + `are/simulation/apps/`), verify signatures, and rewrite using only existing methods.

    CRITICAL: Before calling any email, calendar, contacts, messaging or other app API, use the Read tool to open the PARE app class and its Meta-ARE base
    ...
    - Keep oracle messages (for example, PAREAgentUserInterface.send_message_to_user content or long email bodies) concise and focused on the minimum
...
APPS_AND_DATA_USER_PROMPT = textwrap.dedent(
    ...
    - Use Read to inspect `pare/apps/messaging/app.py` and `are/simulation/apps/messaging_v2.py` before seeding any messaging state.
    ...
    - Use the Read tool to inspect the existing scenario file and any PARE / Meta-ARE app definitions you need.
...
EVENTS_FLOW_USER_PROMPT = textwrap.dedent(
    ...
    - Use Read to inspect the PARE app wrappers under `pare/apps/` for every app you will call in `build_events_flow()`.
    ...
    Use the available agent tools to apply your edits:
    - Use the Read tool to inspect the existing scenario file at {scenario_file_path} and any PARE / Meta-ARE app definitions you need.
